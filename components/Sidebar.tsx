            ...
            <div className="space-y-0">
                ...
                <div className="py-1 bg-[#080808]">
                    {layers.filter(l => l.type === 'GAUSSIAN_SPLAT').map(layer => (
                        <LayerItem key={layer.id} layer={layer} isActive={activeLayerId === layer.id} onSelect={() => onSelectLayer(layer.id)} onToggle={() => onToggleLayer(layer.id)} onDelete={() => onDeleteLayer(layer.id)} icon={<PlayCircle size={12} className={activeLayerId === layer.id ? "text-yellow-500" : "text-yellow-700"} />} />
                    ))}
                </div>
                ...
                <div className="py-1">
                    {layers.filter(l => l.type === 'POINT_CLOUD').map(layer => (
                        <LayerItem key={layer.id} layer={layer} isActive={activeLayerId === layer.id} onSelect={() => onSelectLayer(layer.id)} onToggle={() => onToggleLayer(layer.id)} onDelete={() => onDeleteLayer(layer.id)} icon={<Database size={12} className={activeLayerId === layer.id ? "text-fuchsia-400" : "text-gray-600"} />} />
                    ))}
                </div>
                ...
                <div className="py-1">
                    {layers.filter(l => l.type === 'PHOTO_SET').map(layer => (
                        <LayerItem key={layer.id} layer={layer} isActive={activeLayerId === layer.id} onSelect={() => onSelectLayer(layer.id)} onToggle={() => onToggleLayer(layer.id)} onDelete={() => onDeleteLayer(layer.id)} icon={<Camera size={12} className={activeLayerId === layer.id ? "text-cyan-400" : "text-cyan-700"} />} />
                    ))}
                </div>
            </div>
        )}
...
}> = ({ layer, isActive, onSelect, onToggle, onDelete, icon }) => {
